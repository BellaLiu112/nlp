...
class Sampler:
    ...
    def change_topic(self, doc, index, new_topic):
        ...
        if old_topic != -1:
            assert new_topic == -1, \
                    "doc %i term %i topic already assigned %i, new topic is not -1" % \
                    (doc, index, old_topic)
        ...
        if new_topic != -1:
            assert old_topic == -1, \
                    "doc %i term %i has already been assigned a topic %i" % \
                    (doc, index, old_topic)
    ...
    def sample_probs(self, doc_id, index):
        ...
        doc_normalizer = float(sum(self._doc_counts[doc_id].values())) + float(sum(self._alpha))
        for kk in range(self._num_topics):
            ...
            sample_probs[kk] = (self._doc_counts[doc_id][kk] + self._alpha[kk]) / \
            doc_normalizer * \
            (self._topics._topic_term[kk][term] + self._topics.get_prior(term)) / \
            self._topics.get_normalizer(kk)
...
if __name__ == "__main__":
    ...
    handle = open("topics.txt", 'w')
    lda._topics.report(vocab, handle, 100)
